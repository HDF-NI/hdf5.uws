...
HDF5Interface.prototype.getImageInfo = function(path){
    ...
    path=encodeURIComponent(path);
    xmlHttp.open( "GET", "https://"+location.hostname+(location.port ? ":"+location.port: "")+"/get_image_info/"+path, false ); // false for synchronous request
    xmlHttp.send( );
    return metaData;
}
...
HDF5Interface.prototype.requestImageMosaic = function(path, metaDataInput, read){
        var ws = new WebSocket('wss://'+location.hostname+(location.port ? ":"+location.port: "")+'/read_image_mosaic');
        ws.binaryType = 'arraybuffer';
        ws.addEventListener('open', function open() {
